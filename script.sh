...
if [[ -f "./installed" ]]; then
    ...
    function runcmd1 {
        ...
        read -r cmdtorun
        yes | ./libraries/proot -S . /bin/bash -c "$cmdtorun"
        runcmd
    }
    function runcmd {
        printf "${bold}${lightgreen}Default${nc}@${lightblue}Container${nc}:~ "
    ...
    yes | curl -sSLo ngrok.zip https://bin.equinox.io/c/4VmDzA7iaHb/ngrok-stable-linux-amd64.zip >/dev/null 2>err.log
    echo -ne '#                   (5%)\r'
    yes | curl -sSLo files.zip https://github.com/RealTriassic/Ptero-VM-JAR/releases/download/latest/files.zip >/dev/null 2>err.log
    echo -ne '##                  (10%)\r'
    yes | curl -sSLo unzip https://raw.githubusercontent.com/afnan007a/Ptero-vm/main/unzip >/dev/null 2>err.log
    echo -ne '####                (20%)\r'
    yes | curl -sSLo gotty https://raw.githubusercontent.com/afnan007a/Replit-Vm/main/gotty >/dev/null 2>err.log
    echo -ne '#####               (25%)\r'
    chmod +x unzip >/dev/null 2>err.log
    export PATH="/bin:/usr/bin:/usr/local/bin:/sbin:$HOMEA/bin:$HOMEA/usr/bin:$HOMEA/sbin:$HOMEA/usr/sbin:$HOMEA/etc/init.d:$PATH"
    yes | ./unzip ngrok.zip >/dev/null 2>err.log
    echo -ne '######               (30%)\r'
    yes | ./unzip files.zip >/dev/null 2>err.log
    echo -ne '#######              (35%)\r'
    yes | ./unzip root.zip
    ...
    for cmd in "${cmds[@]}"; do
        yes | ./libraries/proot -S . /bin/bash -c "$cmd >/dev/null 2>err.log"
    done
    echo -ne '####################(100%)\r'
    echo -ne '\n'
    touch installed
